...
import { Loader2, Plus, KeyRound, Copy, Info, Map } from "lucide-react";
...
export default function RegionalManager() {
    ...
    const [newUser, setNewUser] = useState({ full_name: "", email: "", phone: "", role: "REGIONAL_CONVENER", region: "" });
    const [createdCredentials, setCreatedCredentials] = useState<{ email: string, password: string } | null>(null);

    const { toast } = useToast();

    // Guard: Only SUPER_CONTROLLER and NATIONAL_CONVENER
    useEffect(() => {
        if (currentUserRole && !["SUPER_CONTROLLER", "NATIONAL_CONVENER"].includes(currentUserRole)) {
            navigate("/admin");
        }
    }, [currentUserRole, navigate]);

    const fetchAdmins = async () => {
        if (!user || !["SUPER_CONTROLLER", "NATIONAL_CONVENER"].includes(currentUserRole || "")) return;
        ...
        try {
            const { data: rolesData, error: rolesError } = await supabase
                .from("user_roles")
                .select("user_id, role")
                .in("role", ["REGIONAL_CONVENER", "REGIONAL_CO_CONVENER"]);
            ...
            // Optional: If NATIONAL_CO_CONVENER was allowed here, we might not let them see all, but here they can see all.
            setAdmins(combined);
        } catch (error) {
    ...
    async function handleCreateRegionalAdmin() {
        if (!newUser.full_name || !newUser.email || !newUser.region) {
        ...
        try {
            ...
            const { data, error } = await supabase.functions.invoke('provision-user', {
                body: {
                    ...
                    email: newUser.email,
                    phone: newUser.phone,
                    state: newUser.region, // We store the Region name in the State field (e.g. "North Zone")
                    membership_id: membershipId,
                    prant: newUser.region, // We store the Region name in Prant field
                    designation: "Regional Convener",
                    role: newUser.role,
                }
            });
            ...
            toast({ title: "Success", description: "Regional Admin created successfully" });
            fetchAdmins();
        } catch (error: any) {
    ...
    return (
        <div className="p-6 space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold font-display text-white">Regional Managers</h1>
                    <p className="text-muted-foreground mt-1">Assign conveners to oversee specific Zones (e.g., North, South)</p>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-white/5 bg-white/5">
                                ...
                                <th className="p-4 font-medium text-sm text-muted-foreground">Region (Zone)</th>
                            ...
                            {admins.length === 0 ? (
                                <tr>
                                    <td colSpan={5} className="p-8 text-center text-muted-foreground">
                                        No Regional level admins assigned yet.
                                    </td>
                                </tr>
                            ) : admins.map((admin) => (
            ...
            <Dialog open={showCreateDialog} onOpenChange={(open) => {
                if (!open && createdCredentials) {
                    ...
                    setNewUser({ full_name: "", email: "", phone: "", role: "REGIONAL_CONVENER", region: "" });
                }
                setShowCreateDialog(open);
            }}>
                <DialogContent className="sm:max-w-[425px]">
                    <DialogHeader>
                        <DialogTitle>{createdCredentials ? "Admin Created Successfully" : "Assign Regional Admin"}</DialogTitle>
                        <DialogDescription>
                            {createdCredentials
                                ? "Please copy these credentials safely. They cannot be viewed again."
                                : "Create a new Regional Convener or Co-Convener and map them to a Zone."}
                        </DialogDescription>
                    </DialogHeader>
                        ...
                        <div className="space-y-4 py-4">
                            <div className="space-y-2">
                                ...
                                <Select value={newUser.role} onValueChange={(v) => setNewUser(prev => ({ ...prev, role: v }))}>
                                    ...
                                    <SelectContent>
                                        <SelectItem value="REGIONAL_CONVENER">Regional Convener</SelectItem>
                                        <SelectItem value="REGIONAL_CO_CONVENER">Regional Co-Convener</SelectItem>
                                    </SelectContent>
                                </Select>
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <Select value={newUser.region} onValueChange={(v) => setNewUser(prev => ({ ...prev, region: v }))}>
                                    <SelectTrigger>
                            ...
                            <Button onClick={handleCreateRegionalAdmin} className="w-full mt-4" disabled={creating}>
